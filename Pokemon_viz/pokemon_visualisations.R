...
pokemon_raw %>%
  ...
  ggplot() + geom_bar(aes(x = gen, y = avg_total, fill = gen), stat = 'identity') +
...
pokemon_raw %>%
  ggplot() +
  geom_density(aes(x = total, fill = gen)) + 
  facet_wrap(vars(gen), nrow = 4) +
  scale_fill_brewer(palette = 'Dark2')
...
pokemon_avg_gen <- pokemon_raw %>%
  ...
  summarise(hp = median(hp),
            ...
            speed = median(speed)) %>%
  pivot_longer(!gen, names_to = "avg_stat", values_to = "median")

pokemon_avg_gen %>%
  ggplot(aes(x = avg_stat, y = median, fill = gen)) + 
